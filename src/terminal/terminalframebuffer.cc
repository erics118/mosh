...
DrawState::DrawState( int s_width, int s_height )
  : width( s_width ), height( s_height ), cursor_col( 0 ), cursor_row( 0 ), combining_char_col( 0 ),
    ...
    scrolling_region_bottom_row( height - 1 ), renditions( 0 ), hyperlink(), save(),
    cursor_style( Terminal::CursorStyle::BLINKING_BLOCK ), cursor_color(), next_print_will_wrap( false ),
...
void Renditions::set_rendition( color_type num )
{
  ...
  bool value = num < 10;
  /* underline styles are mutually exclusive: selecting one clears the others */
  auto set_only_underline = [this]( attribute_type style ) {
  ...
  switch ( num ) {
    ...
    case 402:
      set_only_underline( underline_double );
      break;
    case 403:
      set_only_underline( underline_curl );
      break;
    case 404:
      set_only_underline( underline_dotted );
      break;
    case 405:
      set_only_underline( underline_dashed );
      break;
    default:
...
{
  if ( num < 0 ) {
    underline_color = 0; /* default / reset */
  } else if ( num <= 255 ) {
    underline_color = 256 + num; /* 256-color index, offset so index 0 is representable */
  } else if ( is_true_color( num ) ) {
    underline_color = num;
  } else {
    underline_color = 0;
...
std::string Renditions::sgr( void ) const
{
  ...
  if ( underline_color ) {
    ...
    if ( is_true_color( underline_color ) ) {
      ...
      snprintf( col, sizeof( col ), "58:5:%d", underline_color - 256 );
    }
    ret.append( col );
  }
  ret.append( "m" );
